  ...
  const embed = new Discord.MessageEmbed()
    ...
    .addField(
      ...
      `${message.guild.members.size} (${
        message.guild.presences.filter(p => p.status === "online").size
      } online, ${
        message.guild.presences.filter(p => p.status === "idle").size
      } idle, ${
        message.guild.presences.filter(p => p.status === "dnd").size
      } dnd, ${
        message.guild.presences.filter(p => p.status === "dnd").size
      } invisible/offline)`,
      true
    ...
    .addField(
      ...
      `${message.guild.roles
        ...
        .join(", ")}`,
      true
    ) // Filter is to filter out @everyone role
