exports.run = async (client, message, args, level) => {
  ...
  message.reply(
    `${member.user.tag} (${member.user.id}) has been warned by ${message.author.tag} (${message.author.id}) because: ${reason}`
  );
  //if (!modlog) {
  ...
  if (modlog)
    modlog
      ...
      .then(() => {
        client.log(
          "log",
          `${message.guild.name}/#${message.channel.name} (${message.channel.id}): ${member.user.tag} (${member.user.id}) was warned by ${message.author.tag} (${message.author.id})`,
          "CMD"
        );
      })
      .catch(err => {
  ...
  member.send({ embed: warnDM }).catch(e => {
    console.log(
      `Unable to DM user \`${member.user.tag} (${member.user.id})\` for warning \`${reason}\` by moderator \`${message.author.tag} (${message.author.id}) \n Error: \n ${e}`
    );
  });
};
...
exports.help = {
  ...
  category: "Moderation",
  description:
    "Warns a user, sends them a message, saves the warning to a mod log channel",
