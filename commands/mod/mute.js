...
exports.run = async (client, message, args, level) => {
  ...
  if (!muteRole) {
    message.guild
      .createRole({
        name: "Muted",
        color: "DARKER_GREY",
        permissions: 36766720
      })
      .then(() =>
        message.reply("As there was no `Muted` role, one was created")
      )
      .catch(() => {
        return message.reply(
          "Unable to create `Muted` role that was not already present"
        );
      });
    muteRole = client.guilds.get(message.guild.id).roles.find("name", "Muted");
  }
  if (message.mentions.users.size < 1)
    return message.reply("You must mention someone to mute them.");
  ...
  if (message.guild.member(user).roles.has(muteRole.id)) {
    message.guild
      ...
      .then(() => {
        message.channel.send(
          `User unmuted by ${message.author.tag} (${message.author.id}) with reason \`${reason}\``
        );
    ...
    message.guild
      ...
      .then(() => {
        message.channel.send(
          `User muted by ${message.author.tag} (${message.author.id}) with reason \`${reason}\``
        );
