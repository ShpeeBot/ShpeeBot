exports.run = async (client, message, args, level) => {
  ...
  if (!age) {
    message.reply(
      "The invite will not expire because no maximum age was specified."
    );
    age = await 0;
  }
  ...
  if (uses.indexOf(".") !== -1) {
    return message.reply(
      "How can you invite only a part of a person? :confused:"
  ...
  message.channel.createInvite({ maxUses: uses, maxAge: age }).then(invite => {
    ...
    message.channel.send(
      `Your invite \`${invite}\` with settings \`maxUses: ${uses}, maxAge: ${age}\``
    );
  });
};
...
exports.help = {
  ...
  category: "Moderation",
  description:
    "Creates an invite to the channel it is sent in. Example usage: `createinvite 1 5m` will create a single use invite valid for 5 mintues",
