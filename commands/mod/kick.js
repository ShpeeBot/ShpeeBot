exports.run = async (client, message, args, level) => {
  ...
  member
    .kick(`${message.author.username} kicked this user with reason: ${reason}`)
    .then(() => {
      message.reply(
        `${member.user.tag} (${member.user.id}) has been kicked by ${message.author.tag} (${message.author.id}) because: ${reason}`
      );
      ...
      if (modlog)
        modlog
          ...
          .then(() => {
            client.log(
              "log",
              `${message.guild.name}/#${message.channel.name} (${message.channel.id}): ${member.user.tag} (${member.user.id}) was kicked by ${message.author.tag} (${message.author.id})`,
              "CMD"
            );
          })
          .catch(err => {
