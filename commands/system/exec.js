...
exports.run = async (client, message, args, level) => {
  ...
  message.reply(
    `Running command \`${code.replace(
      "`",
      "`"
    )}\`... Please wait. _(The exec command **will** terminate the exec process if it takes longer that 15 seconds)_`
  ...
  exec(`${code}`, { timeout: 15000 }, (error, stdout) => {
    ...
    console.log(
      `${message.author.tag} (${message.author.id}) ran console command: \`${code}\``
    );

    if (clean.length > 1800) {
      message.channel
        .send(`${clean}\n\n${Date.now() - time}ms`, { code: "xl", split: true })
        .catch(console.error);
    } else {
