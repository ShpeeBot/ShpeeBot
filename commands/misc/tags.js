exports.run = async (client, message, args, level) => {
  ...
  function list() {
    ...
    tags.forEach(t => {
      ...
      if (!c) return;
      outList += `**${currentTag.tagID.split("-")[1]}**: \`${
        currentTag.content
      }\`\n`;
    });
  ...
  switch (args[0]) {
    case "add":
      ...
      //var content = args.splice(2).join(' ');
      if (!content)
        return message.reply("You need to give me the tag content...");
      //console.log(`${message.guild.id}-${tagName}`);
      if (client.tags.has(`${message.guild.id}-${tagName}`))
        return message.reply("This tag already exists");
      client.tags.set(
        `${message.guild.id}-${tagName}`,
        `{'tagID': '${
          message.guild.id
        }-${tagName}', 'content': '${content}', 'addedBy': '${message.author
          .tag +
          " (" +
          message.author.id +
          ")"}', 'addedAt': '${new Date()}', 'lastModified': '${new Date()}'}`
    ...
    case "edit":
      ...
      //console.log(`${message.guild.id}-${tagName}`);
      if (!client.tags.has(`${message.guild.id}-${tagName}`))
        return message.reply("This tag does not exist");
    ...
    case "delete": //eslint-disable-line no-case-declarations
      ...
      const response = await client.awaitReply(
        message,
        `Are you sure you want to permanently delete ${tagName}? This **CANNOT** be undone.`
      );
      ...
      // If they respond with n or no, we inform them that the action has been cancelled.
        ...
        message.reply(
          "Action cancelled as response was not in the expected format (y/yes/n/no/cancel)"
        );
      }
      break;
    case "info":
...
exports.help = {
  ...
  description:
    "Allows you to add tags (only single-word tags allowed), then run [prefix]tags [tag name] to display the tag, delete the tags if needed, and display more info on the tags",
  usage:
    "Display a tag: tags [tag name]\nAdd a tag: tags add [tag name (single-word only)] [tag text]\nDelete a tag: tags delete [tag name]\nList tags: tags OR tags list\nGet tag info: tags info [tag name]"
};
